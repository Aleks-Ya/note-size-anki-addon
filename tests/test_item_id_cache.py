...
class SizeFormatterTestCase(unittest.TestCase):
    ...
    def test_get_note_size_no_cache(self):
        ...
        act_size_1: SizeBytes = self.size_item_id_cache.get_note_size(note_id, use_cache=False)
        self.assertEqual(exp_size_1, act_size_1)

        content: str = 'updated'
        self.note[self.td.front_field_name] = content
        self.col.update_note(self.note)
        act_size_2: SizeBytes = self.size_item_id_cache.get_note_size(note_id, use_cache=False)
    ...
    def test_get_note_size_use_cache(self):
        ...
        self.note[self.td.front_field_name] = content
        self.col.update_note(self.note)
        act_size_2: SizeBytes = self.size_item_id_cache.get_note_size(note_id, use_cache=True)
