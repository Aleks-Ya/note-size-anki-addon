...
from note_size.types import SizeBytes, SizeStr, SizePrecision


def test_bytes_to_str(size_formatter: SizeFormatter):
    assert size_formatter.bytes_to_str(SizeBytes(0), SizePrecision(1)) == SizeStr("0 B")
    assert size_formatter.bytes_to_str(SizeBytes(456), SizePrecision(1)) == SizeStr("456 B")
    assert size_formatter.bytes_to_str(SizeBytes(1_456), SizePrecision(1)) == SizeStr("1.4 KB")
    assert size_formatter.bytes_to_str(SizeBytes(1_600_456), SizePrecision(1)) == SizeStr("1.5 MB")
    assert size_formatter.bytes_to_str(SizeBytes(1_784_600_456), SizePrecision(1)) == SizeStr("1.7 GB")
    assert size_formatter.bytes_to_str(SizeBytes(1_746_784_600_456), SizePrecision(1)) == SizeStr("1626.8 GB")
...
def __run_bytes_to_str(size_formatter: SizeFormatter):
    for i in range(0, 50_000):
        size_formatter.bytes_to_str(SizeBytes(i), SizePrecision(1))
...
def test_get_cache_size(size_formatter: SizeFormatter):
    ...
    size_formatter.bytes_to_str(SizeBytes(0), SizePrecision(1))
    assert size_formatter.get_cache_size() == 1
    size_formatter.bytes_to_str(SizeBytes(0), SizePrecision(1))
    assert size_formatter.get_cache_size() == 1
    size_formatter.bytes_to_str(SizeBytes(1), SizePrecision(1))
